...
void schedule_offload_and_send(
    ...
    auto offload_and_send_f = [&](int prow, int pcol, int prlen, int pclen) {
        ...
        auto mpi_launcher = [=](auto &&) {
            return hpx::mpi::async(MPI_Isend,
                                   send_ptr,
                                   num_elems,
...
template <typename scalar>
void schedule_blk_load(int num_procs,
                       scalar const *recv_ptr,
                       int prlen,
                       int pclen,
                       int rcv_ld,
                       scalar *cfin_ptr,
...
void schedule_recv_and_load(
    ...
    auto recv_and_load_f = [&](int prow, int pcol, int prlen, int pclen) {
        ...
        recv_futures.clear();
        for (int src_rank = 0; src_rank < num_procs; ++src_rank) {
            auto mpi_fut = hpx::mpi::async(MPI_Irecv,
                                           recv_ptr + src_rank * num_elems,
                                           num_elems,
                                           tsgemm::get_mpi_type<scalar>(),
                                           src_rank,
        ...
        auto load_fut =
            hpx::dataflow(hpx::util::unwrapping(schedule_blk_load<scalar>),
                          num_procs,
                          recv_ptr,
                          prlen,
                          pclen,
